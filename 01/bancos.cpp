...
//#include "include/extrato_mensal.h"
//#include "include/extrato_anual.h"
...
Bancos::Bancos()
{
    Gtk::MenuItem *itm_fechar, *itm_novo_banco, *itm_extrato_mensal, *itm_extrato_anual;
    Gtk::TreeView *listBancos;
    ...
    builder->get_widget("itm_extrato_mensal", itm_extrato_mensal);
    itm_extrato_mensal->signal_activate().connect(sigc::mem_fun(*this, &Bancos::on_itm_extrato_mensal_activate));

    builder->get_widget("itm_extrato_anual", itm_extrato_anual);
    itm_extrato_anual->signal_activate().connect(sigc::mem_fun(*this, &Bancos::on_itm_extrato_anual_activate));
    ...
    cmb_banco->signal_changed().connect(sigc::mem_fun(*this, &Bancos::on_cmb_banco_changed));
...
void Bancos::getBancos()
{
    ...
    auto store_bancos = Glib::RefPtr<Gtk::ListStore>::cast_dynamic(builder->get_object("store_banco"));
    store_bancos->clear();
    ...
    //builder->get_widget("cmb_banco", cmb_bancos);
    cmb_banco->set_active(0);
...
{
    /*Gtk::Calendar *calendar_dataini, *calendar_datafin;
    char sql_totais[1000], dataini[100], datafin[100];
    ...
    sprintf(sql_totais, "SELECT creditos, debitos, creditos - debitos AS saldo FROM (SELECT IFNULL(SUM(valor), 0) AS creditos FROM itens WHERE data  BETWEEN '%s' AND '%s' \
    AND banco = %s AND tpbanc = 1) AS creditos, (SELECT IFNULL(SUM(valor), 0) AS debitos FROM itens WHERE data  BETWEEN '%s' AND '%s' AND tpbanc = 2 AND banco = %s) \
    AS debitos", dataini, datafin, selected_idbanco.c_str(), dataini, datafin, selected_idbanco.c_str());
    
    ExtratoMensal *extmensal = new ExtratoMensal();
    extmensal->create();
    extmensal->set_job_name("Extrato Mensal");
    extmensal->list_itens(sql, sql_totais, dataini, datafin, selected_idbanco);*/
